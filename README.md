...
应用会在菜单栏显示图标，菜单中会显示当前版本并提供“帮助…”。镜像窗口默认以 `2300 × 70` 像素贴住屏幕底边，并且会出现在所有桌面空间。收到新的有效 Touch Bar 画面时窗口会以 100% 不透明度显示，连续 5 秒没有新画面后自动降至 30%。窗口支持直接拖拽边缘调整宽高，程序不会再根据画面帧自动改变窗口大小；拖动窗口背景可以改变位置。从菜单栏图标选择“设置…”可以打开设置窗口，选择“退出 ToubarReplace”可以退出程序。设置中可以选择底部、顶部或屏幕中央，查看或输入当前窗口的宽高像素，并设置镜像帧率。
